...
description: 通过 Claude Relay 的 audit-query 只读 HTTP API 查询、分页、读取调用详情与 artifact，并导出 gzip NDJSON 以分析审计数据。Use when Codex needs to investigate AI request/response history, user or API-key usage, failures, token or cost anomalies, request IDs, audit artifacts, or mentions audit query, audit-query, 审计查询, or 审计导出.
---

# Query Audit API

通过只读 API 检索 Claude Relay 的长期审计数据，并用最小必要范围回答问题。优先使用随 Skill 提供的标准库 CLI，避免重复拼接 curl、泄露 Token 或错误处理分页。
